...
function createRoom(id = crypto.randomUUID().slice(0, 8), options = {}) {
  const room = {
    ...
    visibility: options.visibility || "public", // Added for Private Toggle
    creatorColor: options.creatorColor || "random", // Added for Color Picker
    chess: new Chess(),
    clients: new Map(),
    lastMove: null,
    result: null,
...
function getRole(room) {
  // If playing a bot, honor creator color if not random, otherwise default to white
  if (room.mode === "bot") {
    return room.clients.size === 0 ? (room.creatorColor !== "random" ? room.creatorColor : "white") : "spectator";
  }

  const usedRoles = new Set([...room.clients.values()].map((client) => client.role));
  
  // If this is the first player joining a human match, give them their preferred color
  if (room.clients.size === 0 && room.creatorColor && room.creatorColor !== "random") {
    return room.creatorColor;
  }

  // Otherwise, fill whatever is empty
...
const server = createServer(async (req, res) => {
  ...
  // --- NEW: Added Public Rooms Route ---
  if (url.pathname === "/api/public-rooms") {
    const publicRooms = Array.from(rooms.values())
      .filter(r => r.visibility === "public" && r.mode === "human" && r.clients.size < 2)
      .map(r => ({ id: r.id, players: r.clients.size }));
  ...
  // --- UPDATED: New Room Route to Read New Params ---
  if (url.pathname === "/api/new-room") {
    ...
    // Grab the new UI parameters sent from frontend
    const visibility = url.searchParams.get("visibility") === "private" ? "private" : "public";
    const colorParam = url.searchParams.get("color");
    const creatorColor = ["white", "black", "random"].includes(colorParam) ? colorParam : "random";

    const room = createRoom(undefined, { mode, difficulty, visibility, creatorColor });
    res.writeHead(200, { "Content-Type": "application/json" });
